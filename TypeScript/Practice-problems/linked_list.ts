...
class LinkedList {
    public head: null | MyNode
    public tail: null | MyNode
   ...
   public addToHead(val:unknown): null | MyNode {
        let node = new MyNode(val);
        if (!this.head) {
            this.head = node;
            this.tail = node;
    ...
    public addToTail(val:unknown): null | MyNode {
        let node = new MyNode(val);
        if (!this.tail) {
            this.head = node;
            this.tail = node;
    ...
    public addAtIndex(idx:number, val:unknown): null {
        if (idx === 0) {
            this.addToHead(val);
        }
        else if (idx >= this.length) {
    ...
    public removeFromHead(): null | MyNode {
        if (this.head) {
            this.head = this.head.next;
        }
        return this.head;
    }
    public removeFromTail(): null | MyNode {
        let curr = this.head;
        while (curr && curr.next && curr.next.next) {
            curr = curr.next;
        }
